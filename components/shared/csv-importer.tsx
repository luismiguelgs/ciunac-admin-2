...
export interface CsvImporterProps {
    onUpload: (file: File) => Promise<{ message: string; recordsProcessed?: number }>
    title?: string
    subtitle?: string
    maxSizeMB?: number
...
export default function CsvImporter({
    onUpload,
    title = "Arrastre su archivo CSV aquí",
    subtitle = "o haga clic para seleccionar un archivo",
    maxSizeMB = 10,
    successIcon = <FileSpreadsheet className="h-8 w-8 text-green-500" />,
    defaultIcon = <Upload className="h-8 w-8 text-primary" />,
    onSuccess
}: CsvImporterProps) {
                ...
                {!file ? (
                    <div className="space-y-4">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Tamaño máximo: {maxSizeMB}MB • Formato: CSV
                        </p>
                    </div>
                ) : (
                    <div className="space-y-4">
