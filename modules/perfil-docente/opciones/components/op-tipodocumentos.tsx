...
export default function OpTiposDocumentos() {
    const { data, loading, setData } = usePerfilOpciones<ITipoDocumentosPerfil>(Collection.TiposDocumentosPerfil);
    const [editingRowId, setEditingRowId] = React.useState<string | null>(null)

    const handleRowAdd = () => {
        const id = Math.floor(Math.random() * 9000) + 1000; // Generate a random temp ID
        const newRow: ITipoDocumentosPerfil & { isNew?: boolean } = {
            id,
            nombre: '',
            puntaje: 0,
            isNew: true
    ...
    const handleRowDelete = async (id: number) => {
        if (confirm("¿Confirma borrar el registro?")) {
            await PerfilOpcionesService.deleteItem(Collection.TiposDocumentosPerfil, id);
            setData((old) => old.filter((row) => row.id !== id));
        }
    }

    const handleRowUpdate = async (newRow: ITipoDocumentosPerfil & { isNew?: boolean }) => {
        if (newRow.isNew) {
            const { isNew, ...payload } = newRow;
            const res = await PerfilOpcionesService.newItem(Collection.TiposDocumentosPerfil, payload)
            const created = { ...payload, id: (res as any).id, isNew: false }
            setData((old) => old.map((row) => (row.id === newRow.id ? created : row)))
        } else {
            const { isNew, ...payload } = newRow;
            await PerfilOpcionesService.updateItem(Collection.TiposDocumentosPerfil, payload)
