...
export interface SolicitudDetailsProps {
...
export function SolicitudDetails({
    ...
    const tiposSolicitud = React.useMemo(
        () => filterTiposSolicitudByGroup(tiposCatalogo, tipoGroup),
        [tipoGroup, tiposCatalogo]
    )
    ...
    const estadosSolicitud = React.useMemo(() => {
        return estados.filter(estado => normalizeCatalogText(estado.referencia) === "SOLICITUD")
    }, [estados])
    const estadoOptions = React.useMemo(() => {
        ...
        if (
            currentEstadoId &&
            currentEstadoNombre &&
            normalizeCatalogText(solicitud.estado?.referencia) === "SOLICITUD" &&
    ...
    const certificateAttachmentUrl = solicitud.imgCertEstudio
    const certificateAttachmentIsPdf = certificateAttachmentUrl
        ? certificateAttachmentUrl.split("?")[0].toLowerCase().endsWith(".pdf")
        : false
            ...
            <div className="max-w-6xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
                        ...
                        <div className="p-8 flex flex-col md:flex-row items-center gap-8">
                            <div className="relative">
                                ...
                                <div className="absolute -bottom-1 -right-1 p-1.5 bg-background rounded-full shadow-lg border border-border">
                                    <div className={cn("h-3 w-3 rounded-full", isFinalized ? "bg-green-500" : "bg-blue-500")} />
                                </div>
                            </div>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                            ...
                            <CardContent className="space-y-8">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <SelectField
                                        ...
                                        options={tipoOptions}
                                        loading={loadingTipos && tipoOptions.length === 0}
                                        disabled={!isEditing || loadingTipos}
                                    />
                                    <SelectField
                                        control={form.control}
                                        name="estadoId"
                                    ...
                                    <SelectField
                                        ...
                                        loading={loadingIdiomas && idiomaOptions.length === 0}
                                    ...
                                    <SelectField
                                        ...
                                        loading={loadingNiveles && nivelOptions.length === 0}
                    ...
                    <div className="lg:col-span-4 space-y-6">
                            ...
                            <Card className="overflow-hidden border-none shadow-md group">
                                ...
                                <CardContent className="p-4 bg-muted/20">
                                    <div className="relative rounded-lg border border-border shadow-inner bg-background overflow-hidden min-h-[450px] flex items-center justify-center group-hover:border-primary/30 transition-colors">
                                        {voucherUrl.includes("drive.google.com") ? (
                                            <div className="w-full h-[450px] relative bg-muted/5">
                                                ...
                                                <iframe
                                                    src={voucherUrl.split('?')[0].replace("/view", "/preview").replace("/edit", "/preview")}
                                            ...
                                            <img
                                                src={getGoogleDriveDirectLink(voucherUrl)}
                                                alt="Voucher"
                        ...
                        {showCertificateAttachment && certificateAttachmentUrl ? (
                            <Card className="overflow-hidden border-none shadow-md group">
                                <CardHeader className="bg-muted/50 py-3">
                                    <CardTitle className="text-sm font-bold flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <ImageIcon className="w-4 h-4 text-primary" />
                                            Certificado de estudios adjunto
                                        </div>
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <Button
                                                    ...
                                                    size="icon"
                                                    className="h-7 w-7"
                                                    onClick={() => window.open(certificateAttachmentUrl, "_blank")}
                                                >
                                                    <ExternalLink className="w-3.5 h-3.5" />
                                                </Button>
                                            </TooltipTrigger>
                                ...
                                <CardContent className="p-4 bg-muted/20">
                                    <div className="relative flex min-h-[320px] items-center justify-center overflow-hidden rounded-lg border border-border bg-background shadow-inner transition-colors group-hover:border-primary/30">
                                        {certificateAttachmentUrl.includes("drive.google.com") || certificateAttachmentIsPdf ? (
                                            <iframe
                                                src={certificateAttachmentUrl.includes("drive.google.com")
                                                    ? certificateAttachmentUrl.split('?')[0].replace("/view", "/preview").replace("/edit", "/preview")
                                                    : certificateAttachmentUrl
                                                }
                                                className="h-[320px] w-full border-none"
                                                allow="autoplay"
                                                title="Visor del certificado de estudios"
                                            />
                                        ) : (
                                            <img
                                                src={getGoogleDriveDirectLink(certificateAttachmentUrl)}
                                                alt="Certificado de estudios adjunto"
