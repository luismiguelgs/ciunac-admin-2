...
import { INivel } from "../interfaces/types.interface";
...
const columns: ColumnDef<INivel>[] = [
    {
        accessorKey: "nombre",
        header: ({ column }) => {
...
export default function OpNiveles() {
    const { data, loading, setData } = useOpciones<INivel>(Collection.Niveles);
    ...
    const handleRowDelete = async (id: number) => {
        if (confirm("¿Confirma borrar el registro?")) {
            await OpcionesService.deleteItem(Collection.Niveles, id);
            setData((old) => old.filter((row) => row.id !== id));
        }
    }

    const handleRowUpdate = async (newRow: INivel) => {
        if (newRow.isNew) {
            const res = await OpcionesService.newItem(Collection.Niveles, newRow)
            const created = { ...newRow, id: (res as any).id, isNew: false }
            setData((old) => old.map((row) => (row.id === newRow.id ? created : row)))
        } else {
            await OpcionesService.updateItem(Collection.Niveles, newRow)
