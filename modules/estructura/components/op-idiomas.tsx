...
const columns: ColumnDef<IIdioma>[] = [
    {
        accessorKey: "nombre",
        header: ({ column }) => {
...
export default function OpIdiomas() {
    const { data, loading, setData } = useOpciones<IIdioma>(Collection.Idiomas);
    const [editingRowId, setEditingRowId] = React.useState<string | null>(null)

    const handleRowAdd = () => {
        const id = Math.floor(Math.random() * 90) + 10;
        const newRow: IIdioma = {
            id,
            nombre: '',
            isNew: true
        };
    ...
    const handleRowDelete = async (id: number) => {
        if (confirm("¿Confirma borrar el registro?")) {
            await OpcionesService.deleteItem(Collection.Idiomas, id);
            setData((old) => old.filter((row) => row.id !== id));
        }
    }

    const handleRowUpdate = async (newRow: IIdioma) => {
        if (newRow.isNew) {
            const res = await OpcionesService.newItem(Collection.Idiomas, newRow)
            const created = { ...newRow, id: (res as any).id, isNew: false }
            setData((old) => old.map((row) => (row.id === newRow.id ? created : row)))
        } else {
            await OpcionesService.updateItem(Collection.Idiomas, newRow)
